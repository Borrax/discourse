...
* that is of a user from the database. Error occurences are processed
* true the error logger.
* @param plainPass - The password string in plain text
* @param dbUserPass - The password string taken straight from the db
* @returns a promise that resolves to boolean or null if it has encountered
* an error
*/
export const comparePasswords = async (plainPass: string, dbUserPass: string): Promise<boolean | null> => {
  ...
  const potentialTruePass = await genHashedPass(plainPass, splitDbPass[1])
